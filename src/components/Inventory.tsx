...
const Inventory: React.FC<InventoryProps> = ({ language }) => {
  ...
  const processedInventory = useMemo(() => {
    // Deduplication by hospitalId + bloodType
    const latestMap = new Map<string, BloodInventory>();
    rawInventory.forEach(item => {
      const key = `${item.hospitalId}_${item.bloodType}`;
  ...
  const criticalShortages = useMemo(() => {
    const criticalMap: Record<BloodGroup, { count: number, needed: number }> = {
      ...
      'B-': { count: 0, needed: 30 },
      'A-': { count: 0, needed: 30 },
      'AB-': { count: 0, needed: 15 },
      'A+': { count: 0, needed: 100 },
      'B+': { count: 0, needed: 100 },
      'O+': { count: 0, needed: 100 },
      'AB+': { count: 0, needed: 50 },
    };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700 pb-20">
      {/* Upper Panel: Division Stats & Actions */}
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="flex-1 bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-2">
              <button className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 transition-colors" title="Export PDF">
                <FileText size={18} />
              </button>
              <button className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 transition-colors" title="Export Excel">
                <Download size={18} />
              </button>
            </div>
          </div>
        ...
        <div className="lg:w-96 bg-red-600 rounded-[2rem] p-6 text-white shadow-xl shadow-red-200 relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <button className="w-full bg-white text-red-600 py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-red-50 transition-all active:scale-95 flex items-center justify-center gap-2">
